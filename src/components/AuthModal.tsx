...
import { X, Mail, Lock, User, LogIn, UserPlus, LogOut, CheckCircle2, Sparkles, ShieldCheck } from 'lucide-react';
...
export const AuthModal: React.FC<AuthModalProps> = ({ onNavigateToProfile }) => {
  ...
  const handleSubmitEmail = async (e: React.FormEvent) => {
    ...
    try {
      if (authMode === 'login') {
        ...
        await signupWithEmail(email, password, firstName, lastName, vocalLevel);
      }
  ...
  const handleGoogleAuthSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await loginWithGoogle(googleFirstName, googleLastName, googleEmailInput);
      setGoogleStep(false);
      if (onNavigateToProfile) {
        onNavigateToProfile();
      }
    } catch (err) {
      setErrorMsg('Errore nell\'accesso Google');
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-md flex items-center justify-center p-4">
      <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 sm:p-8 max-w-md w-full shadow-2xl relative space-y-6">
        ...
        {user ? (
          <div className="space-y-6 text-center">
            ...
            <div className="p-4 rounded-2xl bg-slate-950/80 border border-slate-800/80 text-xs text-slate-300 space-y-2 text-left">
              ...
              <div className="flex justify-between items-center text-slate-400">
                ...
                <span className="text-slate-200 font-medium">{user.createdAt}</span>
              </div>
            </div>
          ...
          /* Google OAuth Flow Step */
          <div className="space-y-5">
            <div className="space-y-2 text-center">
              <div className="w-12 h-12 mx-auto rounded-2xl bg-white/10 border border-white/20 flex items-center justify-center p-2">
              ...
              <h3 className="text-xl font-black text-white">Accedi o Registrati con Google</h3>
              <p className="text-xs text-slate-400">
                Inserisci Nome, Cognome e la tua Gmail per creare o collegare l'account in 1 click.
              </p>
            </div>

            <form onSubmit={handleGoogleAuthSubmit} className="space-y-3.5 text-xs sm:text-sm">
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block text-slate-300 font-bold mb-1">Cognome</label>
                  <div className="relative">
                    <User className="w-4 h-4 text-slate-500 absolute left-3.5 top-3" />
                    <input
                      type="text"
              ...
              <div>
                <label className="block text-slate-300 font-bold mb-1">Email Google / Gmail</label>
                <div className="relative">
              ...
              <button
                type="submit"
                disabled={isLoading}
                className="w-full py-3 rounded-2xl bg-gradient-to-r from-sky-500 to-blue-600 hover:from-sky-400 hover:to-blue-500 text-white font-extrabold text-xs sm:text-sm shadow-lg shadow-sky-500/20 flex items-center justify-center gap-2 cursor-pointer transition-all"
              >
                {isLoading ? (
                  <span className="animate-pulse">Connessione Google in corso...</span>
                ) : (
                  <>
                    <span>Crea Account con Google</span>
              ...
              >
                ← Torna all'accesso standard
              </button>
            </form>
          </div>
        ) : (
          /* Email / Password Form & Google OAuth Option */
          <div className="space-y-5">
            ...
            <form onSubmit={handleSubmitEmail} className="space-y-3.5 text-xs sm:text-sm">
              {authMode === 'signup' && (
                <>
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                  ...
                  <div>
                    <label className="block text-slate-300 font-bold mb-1">Percorso / Ruolo Vocale</label>
                    <select
                      value={vocalLevel}
                    ...
                    >
                      <option value="Allievo Francesca">Allievo di Francesca</option>
                      <option value="Allievo Canto Moderno">Allievo Canto Moderno</option>
                      <option value="Cantante Autodidatta">Cantante Autodidatta</option>
                      <option value="Corista / Cantante Jazz">Corista / Cantante Jazz</option>
                    </select>
                  </div>
                </>
              )}
              ...
              >
                {isLoading ? (
                  <span className="animate-pulse">Autenticazione in corso...</span>
                ) : authMode === 'login' ? (
                  <>
                    <LogIn className="w-4 h-4" />
